...
const plugin_id_name = "jspsych-canvas-multi-choice";

/**
 * **canvas-multi-choice**
 *
 * The canvas-multi-choice plugin displays a set of questions with multiple choice response fields. The participant selects a single answer.
 *
 * @author Christian Mott (adapted from Shane Martin)
 */
class CanvasMultiChoicePlugin implements JsPsychPlugin<Info> {
  static info = info;
    ...
    for (let i = 0; i < answers.length; i++) {
      this.jsPsych.pluginAPI.clickTarget(
        display_element.querySelector(
          `#${plugin_id_name}-response-${i}-${trial.questions[i].options.indexOf(answers[i][1])}`
        ),
...
export default CanvasMultiChoicePlugin;
